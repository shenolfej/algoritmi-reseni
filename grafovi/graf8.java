...
public class Main {
    ...
    public static void main(String[] args){
        ...
        int n = scan.nextInt();
        scan.nextLine();
        AdjacencyListGraph<String> materials = new AdjacencyListGraph<>();
        List<String> matList = new ArrayList<>();
        Map<String, String> transformations = new HashMap<>();
        for(int i = 0; i<n; i++){
            String[] str = scan.nextLine().split(" ");
            materials.addVertex(str[0]);
            transformations.put(str[0], str[1]);
            matList.add(str[0]);
        }
        int m = scan.nextInt();
        scan.nextLine();
        for(int i = 0; i<m; i++){
            String[] str = scan.nextLine().split(" ");
            materials.addEdge(str[0],str[1],Integer.parseInt(str[2]));
        }
        int total = 0;
        for(int i = 0; i<matList.size(); i++){
            Map<String, Integer> DJK =  materials.shortestPath(matList.get(i));
            total += DJK.get(transformations.get(matList.get(i)));
        }
