...
public class Main {
    public static void count(AdjacencyListGraph<Integer> graph, List<Integer> vertex,int padnatRouter) {
        graph.removeVertex(padnatRouter);
        vertex.remove(padnatRouter);
        Set<Integer> visited = new HashSet<>();

        int counter =0;
        for (int i = 0; i < vertex.size() ;i++) {
            Stack<Integer> stack = new Stack<>();
            int startVertex = vertex.get(i);//0
            stack.push(startVertex); //0
            while (!stack.isEmpty()) {
                int vertexC = stack.pop(); // 2
                if (!visited.contains(vertexC)) {
                    visited.add(vertexC);
                    //System.out.print(vertex + " ");
                    for (int neighbor : graph.getNeighbors(vertexC)) {
                        if (!visited.contains(neighbor)) {
                            stack.push(neighbor);
                        }
                    }
    ...
    public static void main(String[] args) {
        ...
        int n = input.nextInt();
        input.nextLine();
        List<Integer> vertex = new ArrayList<>(); // site teminja se cuvaaat
        for (int i = 0; i < n; i++) {
            vertex.add(i); // 0 1 2 3 4
        }
        ...
        for (int i = 0; i < m; i++) {
            ...
            int from = Integer.parseInt(vlez[0]);
            int to = Integer.parseInt(vlez[1]);
            graph.addEdge(from, to);
        }
        int padnatRouter= input.nextInt();
        count(graph, vertex,padnatRouter);
    }
}
