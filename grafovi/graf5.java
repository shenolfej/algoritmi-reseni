...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println(countPaths(graph, start, start, end));
    }
    public static int countPaths(AdjacencyListGraph<Integer> graph, int curr, int currSum, int goalSum){
        if(currSum == goalSum) return 1;
        if(currSum > goalSum) return 0;
        int total = 0;
        for(int n : graph.getNeighbors(curr)){
            total += countPaths(graph, n, currSum + n, goalSum);
        }
        return total;
    }
}
